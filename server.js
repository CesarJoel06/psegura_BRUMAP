...
if (!countUsers || countUsers.c === 0){
  ...
  await db.run("INSERT INTO users(username,password,role) VALUES(?,?,?)", ['operario', hp, 'operario']);
  await db.run("INSERT INTO users(username,password,role) VALUES(?,?,?)", ['supervisor', hp, 'supervisor']);
  await db.run("INSERT INTO users(username,password,role) VALUES(?,?,?)", ['administrador', hp, 'administrador']);
...
// CSRF simple con excepción login
function ensureCsrf(req,res,next){
  if (!req.session.csrf) {
    req.session.csrf = Math.random().toString(36).slice(2) + Math.random().toString(36).slice(2);
  }
  next();
}
...
app.get('/csrf', ensureCsrf, (req,res)=> res.json({ token: req.session.csrf || '' }));
app.use(csrfProtect);
...
async function getLock(u){
  const r = await db.get("SELECT username, attempts, locked_until FROM login_lock WHERE username = ?", [u]);
...
async function loginHandler(req,res){
  ...
  if (lock.locked_until && lock.locked_until > now){
    ...
    if (isJson) return res.status(429).json({ error:'locked', waitSec });
  ...
  if(!user || !ok){
    ...
    await saveLock(lock);
    if (isJson) return res.status(401).json({ error:'bad-credentials', attempts: lock.attempts, lockedUntil: lock.locked_until });
    return res.redirect('/?error=1&attempts=' + lock.attempts + '&lockedUntil=' + (lock.locked_until||0));
  }

  lock.attempts = 0; lock.locked_until = 0; await saveLock(lock);
  req.session.user = { id:user.id, username:user.username, role:user.role };
  if (isJson) return res.json({ ok:true, user:req.session.user });
...
app.post('/api/users', requireRole('supervisor'), async (req,res)=>{
  ...
  if (!/^[a-z0-9._-]{3,32}$/.test(uname)) return res.status(400).json({error:'invalid-username'});
  if (!['operario','supervisor','administrador'].includes(role)) return res.status(400).json({error:'invalid-role'});
  const hp = await bcrypt.hash(String(password||'admin'), 10);
  await db.run("INSERT INTO users(username,password,role) VALUES(?,?,?)", [uname, hp, role]);
  res.json({ ok:true });
});
...
app.listen(PORT, ()=> console.log(`BRUMAP v7-reloaded running at http://localhost:${PORT}`));
